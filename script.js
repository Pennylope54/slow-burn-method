const days = [
  {
    title: "Day 1: Begin Gently",
   <p style="text-align:center; font-size:0.95rem;">
Want the full guided program + journal?
</p> <div style="text-align:center; margin:20px 0;">
  <a href="http://moneymapguide.atwebpages.com/" target="_blank" 
  style="display:inline-block; padding:16px 28px; background:#5E6B55; color:white; text-decoration:none; border-radius:30px; font-size:1.1rem; font-weight:500;">
    Start the 30-Day Challenge
  </a>
</div>focus: "Intention: Start softly and simply.",
    instruction: "Walk slowly for 5 minutes. Keep your shoulders relaxed and breathe naturally.",
    prompt: "What did it feel like to slow down?"
  },
  {
    title: "Day 2: Breath and Posture",
    focus: "Intention: Notice your breath and stand tall.",
    instruction: "Walk for 6 minutes. Keep your head lifted and your breath steady.",
    prompt: "How did your body respond to better posture?"
  },
  {
    title: "Day 3: Slowing Down",
    focus: "Intention: Move with calm attention.",
    instruction: "Walk for 7 minutes at a slower pace than usual.",
    prompt: "What changed when you slowed your pace?"
  }
];

for (let i = 4; i <= 30; i++) {
  days.push({
    title: `Day ${i}`,
    focus: "Intention: Stay present and keep going.",
    instruction: `Complete your mindful walking practice for ${Math.min(5 + i, 20)} minutes.`,
    prompt: "What did you notice in your mind and body today?"
  });
}

const MS_PER_DAY = 24 * 60 * 60 * 1000;

const dayTitle = document.getElementById("dayTitle");
const dayFocus = document.getElementById("dayFocus");
const dayInstruction = document.getElementById("dayInstruction");
const dayPrompt = document.getElementById("dayPrompt");
const progressFill = document.getElementById("progressFill");
const progressText = document.getElementById("progressText");
const completeBtn = document.getElementById("completeBtn");
const resetBtn = document.getElementById("resetBtn");
const saveJournalBtn = document.getElementById("saveJournalBtn");
const saveMessage = document.getElementById("saveMessage");

let completedDays = JSON.parse(localStorage.getItem("completedDays")) || [];
let startDate = localStorage.getItem("startDate");

function getTodayLocalMidnight() {
  const now = new Date();
  return new Date(now.getFullYear(), now.getMonth(), now.getDate());
}

function getStartDate() {
  if (!startDate) {
    const today = getTodayLocalMidnight();
    startDate = today.toISOString();
    localStorage.setItem("startDate", startDate);
  }
  return new Date(startDate);
}

function getUnlockedDay() {
  const start = getStartDate();
  const today = getTodayLocalMidnight();
  const diffDays = Math.floor((today - start) / MS_PER_DAY) + 1;

  if (diffDays < 1) return 1;
  if (diffDays > 30) return 30;
  return diffDays;
}

function getCurrentDayToShow() {
  const unlockedDay = getUnlockedDay();
  const nextIncompleteDay = completedDays.length + 1;
  return Math.min(nextIncompleteDay, unlockedDay, 30);
}

function updateProgress() {
  const percent = (completedDays.length / 30) * 100;
  progressFill.style.width = percent + "%";
  progressText.textContent = `${completedDays.length} of 30 days completed`;
}

function loadDay() {
  const unlockedDay = getUnlockedDay();
  const currentDay = getCurrentDayToShow();
  const dayData = days[currentDay - 1];

  updateProgress();

  if (completedDays.includes(currentDay)) {
    dayTitle.textContent = `${dayData.title} ✅`;
    dayFocus.textContent = dayData.focus;
    dayInstruction.textContent = dayData.instruction;
    dayPrompt.textContent = dayData.prompt;
    completeBtn.textContent = "Completed ✅";
    completeBtn.disabled = true;
    return;
  }

  if (currentDay > unlockedDay) {
    dayTitle.textContent = "🔒 Locked";
    dayFocus.textContent = `Your next day unlocks tomorrow.`;
    dayInstruction.textContent = "";
    dayPrompt.textContent = "";
    completeBtn.textContent = "Locked";
    completeBtn.disabled = true;
    return;
  }

  dayTitle.textContent = dayData.title;
  dayFocus.textContent = dayData.focus;
  dayInstruction.textContent = dayData.instruction;
  dayPrompt.textContent = dayData.prompt;
  completeBtn.textContent = "Mark Day Complete";
  completeBtn.disabled = false;
}

completeBtn.addEventListener("click", () => {
  const currentDay = getCurrentDayToShow();
  const unlockedDay = getUnlockedDay();

  if (currentDay > unlockedDay) {
    return;
  }

  if (!completedDays.includes(currentDay)) {
    completedDays.push(currentDay);
    completedDays.sort((a, b) => a - b);
    localStorage.setItem("completedDays", JSON.stringify(completedDays));

    completeBtn.textContent = "Completed ✅";
    completeBtn.disabled = true;
    updateProgress();

    if (completedDays.length === 30) {
      dayTitle.textContent = "Challenge Complete 🎉";
      dayFocus.textContent = "You did it.";
      dayInstruction.textContent = "You completed the Slow Burn Method challenge.";
      dayPrompt.textContent = "What are you most proud of?";
    } else {
      const nowLocked = getCurrentDayToShow() > getUnlockedDay() || completedDays.length >= getUnlockedDay();
      if (nowLocked) {
        dayTitle.textContent = "✅ Today Complete";
        dayFocus.textContent = "Amazing job. Your next day unlocks tomorrow.";
        dayInstruction.textContent = "";
        dayPrompt.textContent = "";
      }
    }
  }
});
saveJournalBtn.addEventListener("click", () => {
  const currentDay = getCurrentDayToShow();
  const entry = document.getElementById("journalEntry").value;
  const mood = document.getElementById("mood").value;
  const energy = document.getElementById("energy").value;
  const stress = document.getElementById("stress").value;

  const journalData = {
    entry,
    mood,
    energy,
    stress
  };

  localStorage.setItem(`journalDay${currentDay}`, JSON.stringify(journalData));
  saveMessage.textContent = "Journal saved ✅";
});

resetBtn.addEventListener("click", () => {
  const confirmed = confirm("Are you sure you want to reset all progress and journal entries?");
  if (!confirmed) return;

  localStorage.removeItem("completedDays");
  localStorage.removeItem("startDate");

  for (let i = 1; i <= 30; i++) {
    localStorage.removeItem(`journalDay${i}`);
  }

  document.getElementById("journalEntry").value = "";
  document.getElementById("mood").value = "";
  document.getElementById("energy").value = "";
  document.getElementById("stress").value = "";
  saveMessage.textContent = "";

  completedDays = [];
  startDate = null;
<div style="text-align:center; margin:20px 0;">
  <a href="http://moneymapguide.atwebpages.com/" target="_blank" 
  style="display:inline-block; padding:16px 28px; background:#5E6B55; color:white; text-decoration:none; border-radius:30px; font-size:1.1rem; font-weight:500;">
    Start the 30-Day Challenge
  </a>
</div>
  loadDay();
});

loadDay();
